...
def create(request : schemas.cinemaSeat, db : Session, id : int, role : str):
    if role != 'admin':
        ...
        hall_list = [id[i].id for i in range(len(check_id)) ]
...
def get_user_cinemaSeat(id: int, request: schemas.cinemaSeat, db: Session):
    return db.query(models.CinemaSeat).filter(models.CinemaSeat.id == id).first()
